...
def train_one_epoch(train_dataloader, model, optimizer, loss):
    ...
    if torch.cuda.is_available():
        ...
        # HINT: use .cuda()
        model.cuda()
    elif torchvision.__version__=="0.14.1":
    ...
    for batch_idx, (data, target) in tqdm(
        ...
        if torchvision.__version__=="0.14.1":
            if torch.backends.mps.is_available():
                output.to('mps')
...
def optimize(data_loaders, model, optimizer, loss, n_epochs, save_path, interactive_tracking=False):
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, "min", verbose=True, threshold=0.01, patience=5) 
    
    for epoch in range(1, n_epochs + 1):
        if epoch>0:
            print( # I inserted this, because the other message is overwritten by the progess bar
                "Last Epoch: {} \tTraining Loss: {:.6f} \tValidation Loss: {:.6f}".format(
